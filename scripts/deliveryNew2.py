...
def odometryCallback(msg):
    ...
    if x*cos(tile_yaw) + y*sin(tile_yaw) > (tile_x*cos(tile_yaw) - 1)*abs(cos(tile_yaw)) + (tile_y*sin(tile_yaw) - 1)*abs(sin(tile_yaw)) and ballistic:
        signalToDrop = 1
...
def generateTraj():
    ...
    P1 = waypointFromWall( 7,               tile_z  + ball_to_uav_offset - 1 + 0.5  )   #back P1
    P2 = waypointFromWall( 3,               tile_z  + ball_to_uav_offset - 1 + 0.5    )   #back P2
    P3 = waypointFromWall( 1.2,          tile_z  + ball_to_uav_offset      + 0 + 0.5 )   #Continue straight from here
    P4 = waypointFromWall( 3,             tile_z  + ball_to_uav_offset + 0.5 )   #end straight path
    P5 = waypointFromWall( 7,               tile_z  + ball_to_uav_offset + 0.5 )   #stopping point
    ...
    V3 = Twist(linear=Vector3( 0,   0,  6),     angular=Vector3( 0, 0, 0))
...
def clostTo(X, Y):
    if abs(X.x - Y.x) <= 0.3 and abs(X.y - Y.y) <= 0.3:
        return True
    else:
        return False
...
def main():
    ...
    if detected:
        ...
        pose_ = waypointFromWall( 7,               tile_z  + ball_to_uav_offset - 1 + 0.5)
